  ...
  return (
    <section className="rounded-[2rem] border border-white/10 bg-white/85 dark:bg-slate-950/80 dark:border-slate-700 shadow-2xl shadow-slate-950/20 backdrop-blur-xl p-6 lg:p-8">
      <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
        <div>
          <p className="text-sm uppercase tracking-[0.4em] text-slate-500 dark:text-slate-400">Motivation</p>
          <h2 className="mt-2 text-3xl font-semibold text-slate-950 dark:text-slate-50">Daily Boost</h2>
          <p className="mt-2 max-w-2xl text-sm leading-6 text-slate-600 dark:text-slate-300">
            Enjoy a rotating wellness card with fresh encouragement, focus tips, and a calm visual mood.
          </p>
        </div>
        <div className="rounded-3xl border border-slate-200/80 dark:border-slate-800/80 bg-slate-50/90 dark:bg-slate-900/80 p-4 text-center shadow-sm">
          ...
          <p className="mt-2 text-xs uppercase tracking-[0.35em] text-slate-500 dark:text-slate-400">New quote every few seconds</p>
        </div>
      </div>
        ...
        <AnimatePresence mode="wait">
          ...
          >
            <div className="flex items-center gap-4">
              ...
              <div>
                <p className="text-xs uppercase tracking-[0.4em] text-slate-500 dark:text-slate-400">{current.label}</p>
                <p className="mt-2 text-xl font-semibold text-slate-950 dark:text-slate-100">Wellness reminder</p>
              </div>
            </div>
            <p className="mt-6 text-lg leading-8 text-slate-700 dark:text-slate-300">{current.text}</p>
          </motion.div>
        </AnimatePresence>
      </div>
    </section>
