    ...
    async (page = 1, append = false) => {
      ...
      try {
        const response = await fetch(
          `/api/conversations?page=${page}&limit=10`
        );
        if (response.ok) {
  ...
  const handleSendMessage = useCallback(
    async (messageText) => {
      ...
      try {
        ...
        if (
          actionResponse &&
          (actionResponse.matched || actionResponse.success)
        ) {
          ...
          const resolvedString = await generateBotResponse(
            text,
            currentCategory,
            idToken,
            [...messages, userMsg]
          ...
          const defaultFallback =
            fallbackResponses[currentCategory] ?? fallbackResponses.general;
          if (!user && resolvedString === defaultFallback) {
            botText =
              "Please [**sign in**](/auth) to use the conversational AI chatbot features.";
          } else {
        ...
        botText =
          "I ran into an unexpected error processing that. Could you try asking again?";
      }
...
>>>>>>> c1d2d10d (Fix chatbot overlap on mobile auth page)
    ...
    >
      {/* Header */}
      <div
        className={`${themeTokens.header} text-white p-4 rounded-t-xl flex items-center justify-between shrink-0 select-none`}
      >
        ...
        <div className="flex items-center space-x-1">
          ...
          >
            <BookOpen
              size={16}
              className={isHistoryOpen ? "text-yellow-300" : "text-white"}
            />
          </button>
          ...
          <button
            ...
            className="hover:bg-white/20 p-2 sm:p-2 rounded-lg transition-colors min-w-[44px] min-h-[44px] flex items-center justify-center cursor-pointer"
            title="Close"
      ...
      {!isMinimized && (
        <>
          ...
          <div className="flex-1 relative overflow-hidden flex">
            ...
            {isHistoryOpen && (
              <div
                className={`absolute inset-0 z-10 flex flex-col border-r shadow-xl animate-in slide-in-from-left duration-250 ${isDarkMode ? "bg-gray-950 border-white/10" : "bg-gray-50 border-gray-200"}`}
              >
                <div className="p-3 border-b flex items-center justify-between font-semibold text-sm border-inherit select-none">
                  <span className="flex items-center gap-1.5 text-purple-400">
                    <BookOpen size={16} /> Activity History Log
                  </span>
                  <button
                ...
                <div className="flex-1 overflow-y-auto p-2 space-y-2 scrollbar-none">
                  {isHistoryLoading && historyPage === 1 ? (
                    ...
                    <div className="text-xs text-gray-500 text-center py-8">
                      No previous structural logs matched under this session.
                    </div>
                  ) : (
                    <>
                      {chatHistory.map((session, index) => (
                        ...
                        >
                          <p className="font-medium truncate text-purple-400 mb-0.5">
                            💬 {session.userMessage}
                          </p>
                          <p className="text-gray-400 dark:text-gray-500 line-clamp-2 text-[11px] leading-relaxed">
                            {session.botMessage}
                          </p>
                        </div>
                      ))}
            ...
            >
              {messages.map((msg) => (
                <div
                  key={msg.id}
                  className={`flex ${msg.isBot ? "justify-start" : "justify-end"} items-start space-x-2.5`}
                >
                  {msg.isBot && (
                    <div
                      className={`p-2 rounded-xl shrink-0 mt-0.5 select-none ${themeTokens.botAvatar}`}
                    >
                      <Bot size={16} />
                    </div>
                  )}
                  <div className="flex flex-col max-w-[82%] group">
                    ...
                    <div
                      className={`p-3 rounded-2xl text-sm leading-relaxed ${msg.isBot ? themeTokens.botMsg : themeTokens.userMsg}`}
                    >
                  ...
                  {!msg.isBot && (
                    <div
                      className={`p-2 rounded-xl shrink-0 mt-0.5 select-none ${themeTokens.userAvatar}`}
                    >
                      <User size={16} />
                    </div>
                  )}
              ...
              {isLoading && (
                <div className="flex justify-start items-center space-x-2.5 animate-pulse select-none">
                  ...
                  <div
                    className={`px-4 py-3 rounded-2xl flex items-center space-x-1.5 ${themeTokens.loading}`}
                  >
                    <span
