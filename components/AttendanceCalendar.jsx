    ...
    <div className="bg-black/40 backdrop-blur-xl rounded-[2rem] border border-white/10 p-6 shadow-2xl transition-all duration-500">
      ...
      <div className="mt-6 grid grid-cols-3 gap-2 text-xs">
        {Object.entries(STATUS_CONFIG).map(([key, config]) => (
          <div
            key={key}
            className={`flex items-center justify-center gap-2 rounded-2xl border px-3 py-2 ${config.bg} ${config.border}`}
          >
            <div
              ...
              <div className="p-6">
                ...
                {modalData ? (
                  <div className="space-y-4">
                    <div
                      className={`flex items-center gap-3 p-4 rounded-2xl border ${STATUS_CONFIG[modalData.status].bg} ${STATUS_CONFIG[modalData.status].border}`}
                    >
                      {React.createElement(
                        STATUS_CONFIG[modalData.status].icon,
                        {
                          className: `w-6 h-6 ${STATUS_CONFIG[modalData.status].color}`,
                        }
                      )}
                      <div>
                        <div
                          className={`font-bold ${STATUS_CONFIG[modalData.status].color} uppercase tracking-wider text-sm`}
                        >
                    ...
                    <div className="bg-black/30 rounded-2xl p-4 space-y-3 border border-white/5">
                      <div className="flex items-center gap-3 text-slate-300">
                        ...
                        <span className="font-medium text-white">
                          {modalData.subject || "General Attendance"}
                        </span>
                      </div>
                      <div className="flex items-center gap-3 text-slate-300">
                  ...
                  <div className="py-8 flex flex-col items-center text-center space-y-3">
                    ...
                    <p className="text-sm text-slate-500">
                      There are no attendance records logged for this specific
                      day.
                    </p>
                  </div>
                )}
