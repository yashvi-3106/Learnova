...
const MOCK_MISTAKE_ANALYSIS = {
  ...
  topics: [
    ...
    {
      name: "Network Security & Phishing",
      incorrect: 2,
      total: 4,
    ...
    {
      name: "Digital Forensics & Cryptography",
      incorrect: 0,
      total: 3,
      status: "Mastered",
  ...
  questions: [
    {
      id: 1,
      question:
        "Which of the following is an execution layer vulnerability often exploited in phishing campaigns?",
      userAnswer: "Static Route Invalidation",
      correctAnswer: "Malicious Macro Script Injection",
      explanation:
        "Phishing emails frequently distribute payloads via document templates containing hidden macro scripts that compile automatically upon file execution paths.",
      ...
      <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-100 flex flex-col md:flex-row items-center justify-between gap-6 mb-8">
        <div>
          <h1 className="text-2xl font-black text-slate-800 tracking-tight">
            Interactive Quiz Performance Engine
          </h1>
          <p className="text-sm text-slate-500 mt-1">
            Post-assessment analytical telemetry and topic-wise breakdown
            metrics.
          </p>
        </div>
        <div className="flex gap-4">
          <div className="px-5 py-3 bg-emerald-50 rounded-2xl border border-emerald-100 text-center">
            <span className="block text-2xl font-bold text-emerald-600">
              {MOCK_MISTAKE_ANALYSIS.correctAnswers}
            </span>
          ...
          <div className="px-5 py-3 bg-rose-50 rounded-2xl border border-rose-100 text-center">
            <span className="block text-2xl font-bold text-rose-600">
              {MOCK_MISTAKE_ANALYSIS.incorrectAnswers}
            </span>
            <span className="text-[11px] text-rose-600 font-semibold uppercase tracking-wider">
              Incorrect
            </span>
          </div>
        </div>
      ...
      {activeTab === "analytics" && (
        <div className="space-y-4">
          {MOCK_MISTAKE_ANALYSIS.topics.map((topic, idx) => (
            <div
              key={idx}
              className="p-5 bg-white border border-slate-100 rounded-2xl shadow-sm flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4"
            >
              <div>
                ...
                <p className="text-xs text-slate-400 mt-0.5">
                  Incorrect responses: {topic.incorrect} out of {topic.total}{" "}
                  tracks
                </p>
              </div>
              <span
                className={`text-xs px-3 py-1 rounded-full font-bold tracking-wide shadow-xs ${topic.color}`}
              >
                {topic.status}
              </span>
            </div>
          ...
          {MOCK_MISTAKE_ANALYSIS.questions.map((q) => (
            ...
            >
              ...
              <div className="mt-4 flex flex-wrap gap-2 items-center">
                <span className="text-[10px] uppercase font-bold text-indigo-400 block mr-2">
                  Recommended Framework Revision:
                </span>
                {q.resources.map((res, rIdx) => (
                  <span
                    key={rIdx}
                    className="bg-indigo-50 text-indigo-700 border border-indigo-100 text-[11px] px-3 py-0.5 rounded-md font-medium cursor-pointer hover:bg-indigo-100 transition-all"
                  >
                    📚 {res}
                  </span>
                ))}
