...
const MOCK_SYLLABUS_TRACKS = {
  ...
  modules: [
    {
      id: "mod-1",
      name: "Architecture Assessment & Tool Setup",
      topicsCount: 5,
      completedTopics: 5,
      weight: "15%",
    },
    {
      id: "mod-2",
      name: "Automated CI/CD Isolation Workflows",
      topicsCount: 8,
      completedTopics: 6,
      weight: "30%",
    },
    {
      id: "mod-3",
      name: "Local Interceptor & Sync Cache Control",
      topicsCount: 6,
      completedTopics: 2,
      weight: "25%",
    },
    {
      id: "mod-4",
      name: "Gamified Retentions & Badge Verification",
      topicsCount: 4,
      completedTopics: 0,
      weight: "30%",
    },
    ...
    <div className="p-6 max-w-4xl mx-auto bg-slate-50 min-h-screen rounded-2xl border border-slate-100">
      ...
      <div className="bg-white p-6 rounded-2xl shadow-xs border border-slate-200/60 mb-8">
        <span className="text-[10px] font-black tracking-widest text-indigo-600 uppercase">
          Syllabus Analytics Engine
        </span>
        <h1 className="text-xl font-bold text-slate-800 mt-1">
          {MOCK_SYLLABUS_TRACKS.courseTitle}
        </h1>
        ...
        <div className="mt-5">
          <div className="flex justify-between text-xs font-semibold text-slate-500 mb-2">
            ...
            <span className="text-indigo-600 font-bold">
              {MOCK_SYLLABUS_TRACKS.overallCompletion}% Done
            </span>
          </div>
      ...
      <div className="space-y-4">
        {MOCK_SYLLABUS_TRACKS.modules.map((mod) => {
          const completionPercentage = Math.round(
            (mod.completedTopics / mod.topicsCount) * 100
          );
              ...
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
                <div>
                  ...
                  <p className="text-xs text-slate-400 mt-1">
                    Completion telemetry: {mod.completedTopics}/
                    {mod.topicsCount} core tasks compiled (Weight: {mod.weight})
                  </p>
                </div>
                <span
              ...
              {selectedModule === mod.id && (
                <div className="mt-4 pt-4 border-t border-slate-100 bg-slate-50/50 p-3 rounded-lg text-xs text-slate-600 leading-relaxed space-y-2">
                  <div className="font-semibold text-slate-700">
                    Sub-topic Extraction Pipeline Diagnostics:
                  </div>
