  ...
  const promise = (async () => {
    ...
    try {
      if (isClient && !navigator.onLine && isMutation) {
        const offlineRes = await handleOfflineRequest(url, { method, body: requestBody, headers: requestHeaders });
      ...
      if (csrfToken && shouldAttachCsrfToken(url, method)) {
        requestHeaders["X-CSRF-Token"] =
          requestHeaders["X-CSRF-Token"] || requestHeaders["x-csrf-token"] || csrfToken;
      }
      ...
      const isNetworkError = errorMsg.includes("Failed to fetch") || err.name === "TypeError" || (typeof navigator !== "undefined" && !navigator.onLine);
