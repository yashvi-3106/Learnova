...
export async function getUserImageFromDb({
  id,
  callerUid,
  callerRole,
  callerInstituteId,
  ...
  if (
    ["teacher", "institute"].includes(callerRole) &&
    callerInstituteId &&
    targetUser.instituteId === callerInstituteId
  ) {
    return targetUser.image;
  }
  ...
  try {
    ...
    if (sumAbs < 1e-4) {
      throw new ValidationError(
        "Invalid face descriptor format: Spoofed or empty face descriptor detected"
      );
    }
...
export async function updateUserImageInDb({
  ...
  const result = await users.updateOne({ firebaseUid }, updateDoc);
