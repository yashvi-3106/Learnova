 ...
 */
async function recordPendingOperation({
  operationId,
  operationType,
  uid,
  steps,
  ...
  try {
    ...
    logger.error(
      "[TransactionCoordinator] Failed to record pending operation:",
      {
        operationId,
        error: err.message,
      }
...
async function updatePendingOperation(operationId, status, metadata = {}) {
  try {
    ...
    logger.error(
      "[TransactionCoordinator] Failed to update pending operation:",
      {
        operationId,
        error: err.message,
      }
...
async function executeCompensation(compensator, description, operationId) {
  for (let attempt = 1; attempt <= MAX_COMPENSATION_RETRIES; attempt++) {
    try {
      await compensator();
      logger.info(
        `[TransactionCoordinator] Compensation succeeded: ${description}`,
        {
          operationId,
          attempt,
        }
      ...
      logger.error(
        `[TransactionCoordinator] Compensation failed (attempt ${attempt}/${MAX_COMPENSATION_RETRIES}): ${description}`,
        {
          operationId,
          error: err.message,
        }
  ...
  logger.error(
    `[TransactionCoordinator] Compensation FAILED after ${MAX_COMPENSATION_RETRIES} attempts: ${description}`,
    {
      operationId,
    }
  );
...
export async function executeSaga({
  ...
  const operationId =
    idempotencyKey || generateIdempotencyKey(operationType, uid);
  ...
  if (idempotencyKey) {
    try {
      ...
      if (existingOp) {
        if (
          existingOp.status === "in_progress" ||
          existingOp.status === "compensating"
        ) {
      ...
      logger.error(
        "[TransactionCoordinator] Error checking existing operation:",
        {
          operationId,
          error: err.message,
        }
...
export async function cleanupOldOperations(maxAgeMs = 7 * 24 * 60 * 60 * 1000) {
  try {
    ...
    const result = await db.collection("pending_operations").deleteMany({
      createdAt: { $lt: cutoff },
      status: { $in: ["completed", "idempotent", "failed"] },
    });
    if (result.deletedCount > 0) {
      logger.info(
        `[TransactionCoordinator] Cleaned up ${result.deletedCount} old pending operations`
      );
    }
  } catch (err) {
