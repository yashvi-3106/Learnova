...
export async function awardXp(firebaseUid, actionType, metadata = {}) {
  ...
  return withLock(`lock:gamification:${firebaseUid}`, async () => {
    ...
    if (actionType === "attendance_marked") {
      const alreadyAwarded = attendanceHistory.some((rec) => rec.date === effectiveDate);
      if (alreadyAwarded) {
        return {
          xpAwarded: 0,
      ...
      if (actionType === "attendance_marked") {
        try {
          await db.collection("xp_awards").updateOne(
            { firebaseUid, actionType: "attendance_marked", date: effectiveDate },
            { $setOnInsert: { firebaseUid, actionType: "attendance_marked", date: effectiveDate, createdAt: now } },
            { upsert: true }
          );
        } catch (_) {
